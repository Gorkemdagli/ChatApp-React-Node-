...
describe('useChatData Hook (ULTRATHINK Tests)', () => {
    ...
    let messagesBuilder: any
    let messageDeletionsBuilder: any
    let roomDeletionsBuilder: any
    let roomsBuilder: any
    let membersBuilder: any
    ...
    beforeEach(() => {
        ...
        mockState = {
            ...
            cacheUsers: vi.fn((users) => {
                if (!users) return
                users.forEach((u: any) => mockState.userCacheRef.current.set(u.id, u))
            }),
            MESSAGE_LIMIT: 20
        }
    ...
    it('should fetch user from DB if not in cache (Caching Strategy)', async () => {
        ...
        usersBuilder.then.mockImplementation((resolve: any) => resolve({ data: dbUser, error: null }))
