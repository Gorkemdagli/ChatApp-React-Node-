...
interface AddFriendModalProps {
  onClose: () => void
  onAddFriend: (code: number) => Promise<{ success: boolean; error?: string }>
  isLoading?: boolean
}
        ...
        <div className="px-6 pb-8 space-y-4 text-center">
          ...
          <div className="pt-2">
            <input
              ...
              className="w-full px-5 py-3.5 rounded-2xl border-[3px] border-[#D0D4F1] text-gray-600 placeholder:text-gray-400 focus:outline-none focus:border-[#7C87EA] transition-all font-medium text-center text-2xl font-mono tracking-wider"
              maxLength={7}
            />
          </div>
          ...
          <button
            ...
            className={`w-full py-4 text-white font-bold rounded-[20px] transition-all active:scale-[0.98] shadow-lg ${isLoading || friendCode.length !== 7
